...
chromosomes_amount = 10
genes_amount = 500
new_child_amount = 4 # n*2
iter = 50
...
#create images
path = r'/Users/idel_isbaev/PycharmProjects/IAI_ass/images/img_02.jpg'
...
# while fit_rate_check > 30000:
    ...
    if repeat_amount == 0 or ki == iter - 1 or fit_rate_check > 20000:
        chromosomes_img1 = [x for _, x in sorted(zip(fit_rate, chromosomes_img))]  # sort
        if repeat_amount == 0:
            plt.subplot(121)
            plt.axis("off")
            ...
            plt.subplot(122)
            plt.axis("off")
            plt.imshow(cv.cvtColor(chromosomes_img1[0], cv.COLOR_BGR2RGB))
            plt.show()
